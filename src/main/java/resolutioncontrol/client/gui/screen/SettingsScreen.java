...
public final class SettingsScreen extends Screen {
	...
	protected void init() {
		...
		doneButton = new ButtonWidget(
			...
			100, buttonSize,
			new LiteralText(I18n.translate("gui.done")),
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float time) {
		...
		drawCenteredString(matrices,getTitle(), centerX, startY + 10, 0x404040);
		
		Text scaleFactor = text("current", mod.getScaleFactor());
		drawCenteredString(matrices,scaleFactor, centerX, centerY - 20, 0x000000);
		
		super.render(matrices, mouseX, mouseY, time); // buttons
	}
	
	private void drawCenteredString(MatrixStack matrices, Text text, int x, int y, int color) {
